...
export default function Card({ isCardClickedFunction, cardClicked, gameDifficulty }) {
  console.log("the card clicked state is ", cardClicked);
...
const isGameEasy = gameDifficulty === "Easy" ? easySizeSettings : hardSizeSettings
  ...
  useEffect(() => {
    ...
    fetcher().then((result) => {
      // console.log("the image is ", result.sprites);
      setPokeData({
        name: result.name,
      ...
      <div className={` box-contents ease-in duration-200`}>
        <div className={`card relative ${cardClicked ? "flipped" : ""}`}>
          <div className={`bg-[#ffc300] card-face card-front ${isGameEasy} rounded-md overflow-hidden absolute`}>
            <img src={pokeData.image} alt="" width={"2000px"} />
